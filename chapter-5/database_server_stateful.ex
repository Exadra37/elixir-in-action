defmodule DatabaseServer do
  @moduledoc """
  [Chapter 5 - Concurrency primitives > 5.3 Stateful server processes > 5.3.2 Keeping a process state](https://livebook.manning.com/book/elixir-in-action-second-edition/chapter-5/point-9313-161-161-0).
  ...
  Check the code difference between the stateless and stateful implementations
  with:

  ```
  git diff --no-index chapter-5/database_server_stateless.ex chapter-5/database_server_stateful.ex
  ```
  ...
  def start() do
    spawn(
      fn ->
        "DSN=db.example.com;UID=john_doe;PWD=the-secret"
  ...
  to trigger a rotation of the empty `active_pool` with the `initial_pool`:
      ...
      [
        "Database Connection 54: query 1 result",
        "Database Connection 657: query 2 result",
        "Database Connection 823: query 3 result",
        "Database Connection 54: query 4 result"
      ]
  ...
  with the `initial_pool`:
      ...
      "Database Connection 9: query 1 result"
  ...
  ## Examples
      ...
      [
        "Database Connection 699: query 1 result",
        "Database Connection 699: query 2 result",
        "Database Connection 699: query 3 result"
      ]
  ...
  ## Examples
      ...
      iex> DatabaseServer.get_result()
      "Database Connection 101: query 1 result"
  ...
  defp _loop(database_connection) do
    receive do
      {:run_query, caller, query_def} ->
      send(caller, {:query_result, _run_query(database_connection, query_def)})
    end

    _loop(database_connection)
  end

  defp _run_query(database_connection, query_def) do
    Process.sleep(2000)
    "Database Connection #{database_connection}: #{query_def} result"
  end

end


defmodule DatabaseDriver do
  def connect(_connection_string, _opts \\ %{}) do
    # In real life it would establish the connection to the database.
    :rand.uniform(1000)
  end
end
